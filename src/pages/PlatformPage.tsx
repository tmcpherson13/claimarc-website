...
interface Module {
  ...
  audience: string;
  required: boolean;
  /** Extra detail revealed when the card is expanded. */
  detail: string;
  /** 3–4 short capability bullets revealed when expanded. */
  capabilities: string[];
...
const ModuleCard = ({ m }: { m: Module }) => {
  ...
  const panelId = `module-panel-${m.name.toLowerCase()}`;
  return (
    <div
      className={`group border rounded-xl bg-white transition-all duration-300 ${
        open
          ? "border-[var(--emerald)] shadow-lg shadow-emerald-100"
          : "border-[var(--lgray)] hover:border-emerald-200 hover:shadow-md"
      }`}
    >
      <button
        type="button"
        ...
        onClick={() => setOpen((v) => !v)}
        className="w-full text-left p-6 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--emerald)] rounded-xl"
      >
        <div className="flex items-start justify-between gap-3">
          <BADBadge required={m.required} />
          <ChevronDown
            className={`h-5 w-5 text-slate-400 transition-transform duration-300 ${
              open ? "rotate-180 text-[var(--emerald)]" : ""
            }`}
            aria-hidden="true"
          />
        </div>
        ...
        {!open && (
          <p className="text-[var(--emerald)] text-xs font-semibold mt-4 inline-flex items-center gap-1">
            What it does
      ...
      >
        <div className="overflow-hidden">
          <div className="px-6 pb-6 border-t border-[var(--lgray)] pt-5">
            <p className="text-slate-700 text-sm leading-relaxed">{m.detail}</p>
            ...
            <Link
              to={m.learnMoreHref}
              className="mt-5 inline-flex items-center gap-1 text-[var(--emerald)] font-semibold text-sm hover:gap-2 transition-all"
            >
